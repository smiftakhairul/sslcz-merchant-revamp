...
import { getParseFloat, getStatusColumnContentByNumber, listGroupItem } from "helpers/common";
        ...
        return <React.Fragment>
            <p className="mb-0">
                <span className="text-muted display-7">
                    {props.transaction.created_on ? moment(props.transaction.created_on, 'YYYY-MM-DD hh:mm:ss').format('DD-MM-YYYY hh:mm A').toString() : ''}
                    &nbsp;|&nbsp;
                    {getParseFloat(props.transaction?.amount || 0)} {props.transaction?.currency || ''}
                </span>
            </p>
        </React.Fragment>
    };
    ...
    return (
        <React.Fragment>
            <ModalWrapper
                isOpen={props.isOpen}
                toggle={props.toggle}
                modalTitle="Recurring Payment Details"
