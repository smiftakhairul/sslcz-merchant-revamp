...
const SearchChargeback = (props) => {
  ...
  const getFields = async () => {
    setIsLoading(true);
    try {
      await apiClient("chargebackRequestSearchFields", {}, {}).then(
        (response) => {
          if (
  ...
  const downloadCsv = async (formData = {}) => {
    ...
    try {
      await apiClient("chargebackRequestDetailsCSVDownload", params, body).then(
        (response) => {
          if (
            response.data.code === 200 ||
            response.data.status === "SUCCESS"
...
export default SearchChargeback;
