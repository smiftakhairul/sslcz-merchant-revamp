...
const recurringListColumns = (currentPage, pageSize, modal) => [
    {
        Header: "#",
          ...
          Cell: props => {
              return (
                  <div>
                        ...
                        <p className="mb-0">Created On: <span className="font-weight-bold">
                            {props.row.original?.created_on ? moment(props.row.original?.created_on, 'YYYY-MM-DD hh:mm:ss').format('DD-MM-YYYY hh:mm A').toString() : ''}
                        </span></p>
                        <p className="mb-0">SMS IPN: <span className="font-weight-bold">{props.row.original?.sms_ipn}</span></p>
                  </div>
            )
        }
    },
            ...
            return (
                <div>
                    ...
                    <Button
                        ...
                        color=""
                        title="Details"
                        onClick={()=>modal(props.row.original, 'RecurringDetail')}
                    >
                        <span className="align-middle d-flex align-items-center"><i className="glyph-icon simple-icon-eye"></i></span>
                    </Button>
                    <Link
                        to={'/app/recurring/update-recurring/' + props.row.original.id}
...
export default recurringListColumns;