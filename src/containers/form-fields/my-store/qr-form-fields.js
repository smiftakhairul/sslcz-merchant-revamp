...
export const qrCustomizedFields = (namePrefix = "", tFields = []) => {
  return [
    {
      label: "Field Type",
      checkboxLabel: "",
...
export const qrMainFieldsPay = (data = null) => {
  return [
    {
      label: "Name of the Payment",
      type: "text",
...
export const qrMainFieldsTransaction = (data = null) => {
  return [
    {
      label: "Amount (BDT)",
      type: "number",
    ...
    {
      label: "Product List with Amount",
      infoText: (
        <div>
          {`* Syntax: AMOUNT|Product Name (Amount); Example: 200|Product Name 1 (200 BDT)<NEW LINE>500|Product Name 2 (500 BDT).`}
          <br />
          <br />
          {`Here- "|" is the amount and product name divider and "<NEW LINE>" means enter another line for another product. Please do not use "|", ":" and ";" within your text. And AMOUNT must be unique`}
          <br />
          <br />
          {`(Only applicable for Payment Link)`}
        </div>
      ),
      type: "textarea",
      name: "product_list",
      id: "product_list",
      placeholder: "Enter Amount List",
      defaultValue: data?.product_list || "",
      required: false,
      height: 100,
      wrapperClass: "col-md-4",
    },
    {
      label: "Item or Product Category",
      type: "text",
      infoText: (
        <div>
          {`* This category field depends on "Product List with Amount". If no item is added in "Product List with Amount", then this category will not display. Category name must be mentioned by comma "," separated. Ex, Small,Regular`}
          <br />
          <br />
          {`This field search the name from Product List. If you give a item name as "Creamy Hot Chocolate (Regular)", then this item will be displayed in "Regular" category automatically. Two more category will be add "All" and "Others"`}
          <br />
          <br />
          {`(Only applicable for Payment Link)`}
        </div>
      ...
      name: "item_category",
      id: "item_category",
      placeholder: "Enter Item or Product Category List by Comma Separated",
      required: false,
      defaultValue: data?.item_category || "",
      wrapperClass: "col-md-4",
    },
    {
    ...
    },
    {
      ...
      options: [
        { label: "Payment Link", value: "once" },
        { label: "Recurring/Subscription", value: "subscribe" },
        {
          label:
            "Invoice - Generate Invoice Link to Collect Fixed Amount",
          value: "invoice",
        },
      ],
      required: true,
      placeholder: "Select option",
      defaultValue: data?.subscription_type,
      wrapperClass: "col-md-4",
    },
    {
      label: "Amount Type",
...
export const qrMainFieldsAddMessage = (data = null) => {
  return [
    {
...
export const qrMainFieldsPageStructure = ( data = null, fileChangeHandler) => {
  return [
    {
